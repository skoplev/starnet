...
getOptimalBeta = function(con_eval, powers, beta_min=0.85, beta_frac=0.95, selBeta) {
	...
	betas = lapply(con_eval, function(x) {
		...
		beta = unlist(x$fitIndices[2, include_powers])
...
load('~/GoogleDrive/projects/STARNET/cross-tissue/co-expression/determinePower/output/con_eval_v2auto.RData')
...
pdf("plots/beta_series_v2auto.pdf", height=9, width=5)
par(mfrow=c(4, 2))
...
pdf("plots/beta_series_v2auto_tissue_slope.pdf", height=16, width=5)
